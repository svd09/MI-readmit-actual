...
a2 = bal.tab(hhc ~  hosp_nrd + age + aweekend + female
            ...
            ,data = df6,
            weights = "wts1",
            distance = "p.score", method = "weighting", un = TRUE)
...
covnames = c("hosp_nrd", 'age' , 'aweekend' , 
'female' , 'cm_chf' , 'cm_chrnlung' ,
'diabetes' , 'cm_htn_c' , 'cm_hypothy' ,
'cm_liver' , 'cm_obese' , 'cm_perivasc',
'cm_renlfail' , 'cancer' , 'hosp_loc' ,
'insu_medicaid', "insu_medicare" ,"insu_others",
"insu_private",  'smoke' , 'year_2012' ,
"year_2013","year_2014",
'dys' , 'priormi' , 'treatment_cabg' ,
"treatment_medical", "treatment_pci", 'priorpci' ,
'car','pstroke')
...
df6$discharge = with(df6, ifelse(hhc == 0, "home", "hhc"))

svy = svydesign(data = df6, strata = ~nrd_stratum, ids = ~hosp_nrd,
                weights = ~discwt + wts1, nest = T)


# adjusted results

adjusted = svyglm(readmit ~ discharge, design = svy, 
...
unadjust = svydesign(data = df6, strata = ~nrd_stratum, ids = ~hosp_nrd,
                weights = ~discwt, nest = T)

unadj = svyglm(readmit ~ discharge, design = unadjust,
               family = "binomial"(link = "log"))
